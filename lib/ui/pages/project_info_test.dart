...
class _ProjectInfoTestPageState extends State<ProjectInfoTestPage> {
  ...
  Future<void> _fetchProjects({bool useExpand = true}) async {
    if (User.cBPartnerID == null) {
      setState(
        () => _debugInfo = 'C_BPartner_ID es null. No se puede consultar.',
      );
      return;
    }
  ...
  Future<void> _fetchProductChip() async {
    try {
      final response = await http.get(
        Uri.parse(
          '${Endpoint.productChip}?\$filter=C_BPartner_ID eq ${User.cBPartnerID}',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': Token.token,
  ...
  Future<void> _fetchOrders() async {
    try {
      final response = await http.get(
        Uri.parse(
          '${Endpoint.order}?\$filter=AD_User_ID eq ${User.userID}&\$expand=C_OrderLine',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': Token.token,
  ...
  Future<void> _fetchRequests() async {
    try {
      final response = await http.get(
        Uri.parse(
          '${Endpoint.request}?\$filter=C_BPartner_ID eq ${User.cBPartnerID}&\$top=20',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': Token.token,
      ...
      child: Scaffold(
        ...
        body: _isLoading
            ...
            : TabBarView(
                children: [
                  _buildProjectList(),
                  _buildProjectPhasesTab(),
                  _buildDebugList(_productChipList),
                  _buildDebugList(_orderList),
                  _buildDebugList(_requestList),
                ],
              ),
      ),
    );
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton(
              onPressed: () => _fetchProjects(useExpand: true),
              child: const Text('Recargar'),
            ),
            const SizedBox(height: 8),
            TextButton(
              onPressed: () => _fetchProjects(useExpand: false),
              child: const Text('Intentar sin Expandir (Debug)'),
            ),
          ],
        ),
      );
          ...
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text(
                  'Tercero: ${project['C_BPartner_ID']?['identifier'] ?? 'N/A'}',
                ),
                const SizedBox(height: 4),
                Text(
                ...
                Text(
                  'ProjectLineLevel: ${project['ProjectLineLevel']?['identifier'] ?? 'N/A'}',
                ),
                const SizedBox(height: 4),
                Text(
                  'Moneda: ${project['C_Currency_ID']?['identifier'] ?? 'N/A'}',
                ),
                Text(
                  'Dirección: ${project['C_BPartner_Location_ID']?['identifier'] ?? 'N/A'}',
                ),
                Text(
                  'Término Pago: ${project['C_PaymentTerm_ID']?['identifier'] ?? 'N/A'}',
                ),
                Text(
                  'Usuario: ${project['AD_User_ID']?['identifier'] ?? 'N/A'}',
                ),
              ],
            ),
            isThreeLine: true,
          ...
          child: ExpansionTile(
            ...
            subtitle: Text(
              ...
              style: TextStyle(
                color: lineLevel == 'P'
                    ? Colors.blue
                    : (lineLevel == 'A' ? Colors.orange : Colors.green),
            ...
            children: [
              ...
              if (phases.isEmpty && directTasks.isEmpty)
                const Padding(
                  ...
                  child: Text(
                    'Este proyecto no tiene fases ni tareas registradas.',
                  ),
                ),
            ],
          ),
            ...
            Padding(
              ...
              child: Column(
                children: [
                  _buildRow(
                    'ID Fase',
                    getValue(phase['C_Phase_ID'] ?? phase['id']),
                  ),
