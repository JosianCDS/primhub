...
class _GanttContentState extends State<GanttContent> {
      ...
      if (startStr != null && startStr.isNotEmpty) {
        ...
        if (start != null) {
          ...
          if (req['C_BPartner_ID'] is Map) {
            bpDescription = (req['C_BPartner_ID']['Description'] ?? req['C_BPartner_ID']['description'] ?? '').toString().trim();
          }
          if (bpDescription.isEmpty) {
            ...
            if (bpId != null) {
              ...
              if (found.isNotEmpty) {
                bpDescription = (found['Description'] ?? found['description'] ?? '').toString().trim();
              }
            }
          }
  ...
  void _applyTimeFilter() {
    ...
    _processedRequests.sort((a, b) => a['_parsedStart'].compareTo(b['_parsedStart']));
  ...
  void _scrollToToday() {
     ...
     Future.delayed(const Duration(milliseconds: 100), () {
        ...
        final offsetPixels = (offsetDays * _dayWidth) - (_horizontalController.position.viewportDimension / 2) + (_dayWidth / 2);
  ...
  void _showRequestDetails(BuildContext context, Map<String, dynamic> req) {
    final rawStatusName = req['R_Status_Name'] ?? (req['R_Status_ID'] is Map ? req['R_Status_ID']['identifier'] : '');
    final statusName = rawStatusName.toString().replaceFirst(RegExp(r'^\d+_'), '');
          ...
          content: Column(
            ...
            children: [
              ...
              if (summary != description) ...[
                Text(_stripHtml(summary), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ...
                    children: [
                      ...
                      PopupMenuButton<String>(
                        ...
                        itemBuilder: (context) => const [
                          ...
                          PopupMenuItem(value: 'next_15_days', child: Text('Bi Semanal')),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                _timeFilter == 'all' ? 'Todo' : _timeFilter == 'this_week' ? 'Semanal' : _timeFilter == 'next_15_days' ? 'Bi Semanal' : 'Mensual',
                              ...
                              return InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Column(
                                    ...
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      if (req['bpDescription']?.toString().isNotEmpty == true)
                          ...
                          Container(
                            ...
                            child: Row(
                                children: List.generate(totalDays, (index) {
                                  final date = _minDate!.add(Duration(days: index));
                                    ...
                                    decoration: BoxDecoration(
                                      border: Border(
                                        right: BorderSide(
                                          ...
                                          width: date.weekday == DateTime.sunday ? 1.5 : 1.0,
                                        ),
                                      ),
                                      color: isToday 
                                          ? colorScheme.primary.withOpacity(0.1) 
                                          : (isWeekend ? colorScheme.surfaceContainerHighest.withOpacity(0.5) : null),
                                    ),
                                    child: Column(
                                      mainAxisAlignment: MainAxisAlignment.center,
                                      children: [
                                        Text(_getWeekdayAbbr(date.weekday), style: TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: isWeekend && !isToday ? colorScheme.outline : colorScheme.primary)),
                                        Text('${date.day}', style: TextStyle(fontWeight: isToday ? FontWeight.bold : FontWeight.bold, fontSize: 13, color: isWeekend && !isToday ? colorScheme.outline : null)),
                                        Text(_getMonthAbbr(date.month), style: TextStyle(fontSize: 9, color: isWeekend && !isToday ? colorScheme.outline : null)),
                                      ],
                                    ),
                                  );
                                }),
                                  ...
                                  Positioned.fill(
                                    child: CustomPaint(
                                      painter: GanttGridPainter(
                                        totalDays: totalDays,
                                        dayWidth: _dayWidth,
                                        lineColor: colorScheme.outlineVariant.withOpacity(0.1),
                                        minDate: _minDate!,
                                        weekendColor: colorScheme.surfaceContainerHighest.withOpacity(0.2),
                                      ),
                                    ),
                                  ),
                                  ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      final startOffset = start.difference(_minDate!).inDays;
                                      final duration = end.difference(start).inDays + 1;
                                      final priority = req['Priority'] is Map ? req['Priority']['identifier'] : (req['Priority'] ?? 'Media');
                                      final color = _getPriorityColor(priority);

                                      String tooltipSummary = _stripHtml(req['Summary'] ?? '');
                                      if (tooltipSummary.length > 400) {
                                        tooltipSummary = '${tooltipSummary.substring(0, 400)}...';
                                      }
                                      ...
                                      return Container(
                                        ...
                                        child: Stack(
                                          children: [
                                            // Barra de Gantt
                                            Positioned(
                                              left: startOffset * _dayWidth + 2, // Margen ligero
                                              width: (duration * _dayWidth) - 4, // Margen ligero
                                              top: 10,
                                              bottom: 10,
                                              child: Tooltip(
                                                message: 'Ticket: ${req['DocumentNo'] ?? ''}\n$tooltipSummary',
                                                child: InkWell(
                                                  onTap: () => _showRequestDetails(context, req),
                                                  child: Container(
                                                  decoration: BoxDecoration(
                                                  ...
                                                  child: Text(
                                                    req['bpDescription']?.toString().isNotEmpty == true 
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < totalDays; i++) {
      ...
      if (date.weekday == DateTime.saturday || date.weekday == DateTime.sunday) {
        canvas.drawRect(Rect.fromLTWH(x, 0, dayWidth, size.height), weekendPaint);
      }
      
      // Dibujar línea divisoria más fuerte al final de la semana (Domingo)
      if (date.weekday == DateTime.sunday) {
         final darkPaint = Paint()..color = lineColor.withOpacity(0.4)..strokeWidth = 1.5;
         canvas.drawLine(Offset(x + dayWidth, 0), Offset(x + dayWidth, size.height), darkPaint);
      } else {
         canvas.drawLine(Offset(x + dayWidth, 0), Offset(x + dayWidth, size.height), linePaint);
      }
    }
  }
