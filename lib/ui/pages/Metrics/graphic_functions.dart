...
class ProjectMetricsCalculator {
  ...
  static String getComplianceCategory(Map<String, dynamic> req) {
    final rawGroup = req['R_Group_ID'];
    final groupId = rawGroup is Map ? rawGroup['id']?.toString() ?? '' : rawGroup?.toString() ?? '';
    if (groupId != '1000006') {
      // Si no es un requerimiento de cliente, no aplica para este gráfico.
      return 'PENDIENTE';
    }
    ...
    String rawStatusName = statusData is Map ? (statusData['Name'] ?? statusData['identifier'] ?? req['R_Status_Name'] ?? 'Sin Estado') : (req['R_Status_Name'] ?? 'Sin Estado');
    String lowerStatus = rawStatusName.toLowerCase();

    if (lowerStatus.contains('espera de cliente')) {
      return 'ESPERA DE CLIENTE';
    } else if (lowerStatus.contains('por entregar') || lowerStatus.contains('evaluacion') || lowerStatus.contains('anulada') || lowerStatus.contains('archivada') || lowerStatus.contains('aprobada por el cliente') || lowerStatus.contains('close') || lowerStatus.contains('cerrad') || lowerStatus.contains('entregado') || lowerStatus.contains('entregada') || statusId == 103 || statusId == 1000019 || statusId == 1000030) {
      return 'TERMINADA';
    } else {
      return 'PENDIENTE';
    }
  ...
  static SimpleChartData _calculateComplianceData(List<Map<String, dynamic>> requests) {
    ...
    final complianceLabels = compliancePieData.keys.where((k) => compliancePieData[k]! > 0).toList();
    final values = complianceLabels.map((k) => compliancePieData[k]!.toDouble()).toList();
    return SimpleChartData(labels: complianceLabels, values: values);
  }

  /// Lógica para el gráfico de Solicitudes por Estado.
  static SimpleChartData _calculateStatusData(List<Map<String, dynamic>> requests) {
    final Map<String, int> statusCounts = {};

    for (var req in requests) {
      final rawGroup = req['R_Group_ID'];
      final groupId = rawGroup is Map ? rawGroup['id']?.toString() ?? '' : rawGroup?.toString() ?? '';
      if (groupId != '1000006') continue;

      final statusData = req['R_Status_ID'] as Map?;
      final rawStatusName = statusData?['Name'] ?? statusData?['identifier'] ?? req['R_Status_Name'] ?? 'Sin Estado';
      final lowerStatus = rawStatusName.toLowerCase();

      if (lowerStatus.contains('anulada')) continue; // No se toman en cuenta las anuladas

      final statusName = rawStatusName.contains('_') ? rawStatusName.split('_').last.trim() : rawStatusName.trim();
      statusCounts[statusName] = (statusCounts[statusName] ?? 0) + 1;
    }

    return SimpleChartData(labels: statusCounts.keys.toList(), values: statusCounts.values.map((v) => v.toDouble()).toList());
  }

  /// Lógica para los gráficos de Módulos.
  static Map<String, dynamic> _calculateModuleMetrics(List<Map<String, dynamic>> requests) {
    ...
    for (var req in requests) {
      ...
      final categoryName = categoryData?['Name'] ?? categoryData?['identifier'] ?? 'Sin Módulo';
      ...
      moduleStatusCounts[categoryName]![modCat] = (moduleStatusCounts[categoryName]![modCat] ?? 0) + 1;
    }

    final moduleFullLabels = moduleStatusCounts.keys.toList();
    final moduleLabels = moduleFullLabels.map((l) => l.replaceAll(RegExp(r'Módulo de |Módulo ', caseSensitive: false), '').trim()).map((l) => l.length > 10 ? '${l.substring(0, 9)}..' : l).toList();
    final moduleTerminadaValues = moduleFullLabels.map((mod) => (moduleStatusCounts[mod]!['TERMINADA'] ?? 0).toDouble()).toList();
    final modulePendienteValues = moduleFullLabels.map((mod) => (moduleStatusCounts[mod]!['PENDIENTE'] ?? 0).toDouble()).toList();
    final moduleEsperaValues = moduleFullLabels.map((mod) => (moduleStatusCounts[mod]!['ESPERA DE CLIENTE'] ?? 0).toDouble()).toList();
    final modulePercentageValues = moduleFullLabels.map((mod) {
      final total = (moduleStatusCounts[mod]!['TERMINADA'] ?? 0) + (moduleStatusCounts[mod]!['PENDIENTE'] ?? 0) + (moduleStatusCounts[mod]!['ESPERA DE CLIENTE'] ?? 0);
      return total > 0 ? ((moduleStatusCounts[mod]!['TERMINADA'] ?? 0) / total * 100) : 0.0;
    }).toList();

    return {
      'moduleStatusData': StackedBarChartData(labels: moduleLabels, fullLabels: moduleFullLabels, seriesValues: [moduleTerminadaValues, modulePendienteValues, moduleEsperaValues], seriesNames: const ['Terminada', 'Pendiente', 'Espera de Cliente']),
      'modulePercentageData': SimpleChartData(labels: moduleLabels, values: modulePercentageValues),
    };
  }
}
