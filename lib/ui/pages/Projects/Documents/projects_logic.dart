...
class ProjectsLogic {
  Future<List<dynamic>> fetchProjects(BuildContext context) async {
    String url = '${Endpoint.project}?\$expand=C_ProjectPhase(\$expand=C_ProjectTask)';
    ...
    try {
      ...
      if (response.statusCode == 401) {
        ...
        if (refreshed) {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sesión expirada. Por favor inicie sesión nuevamente.'), backgroundColor: Colors.red));
            context.go('/login');
          }
          return [];
        }
  ...
  Future<List<dynamic>> fetchBPartners() async {
    try {
      final response = await http.get(Uri.parse('${Endpoint.cBPartner}?\$select=id,Name,Value&\$orderby=Name'), headers: {'Content-Type': 'application/json', 'Authorization': Token.token});
      if (response.statusCode == 200) {
        final data = json.decode(utf8.decode(response.bodyBytes));
        return data['records'] ?? [];
      }
  ...
  Future<List<dynamic>> fetchProjectTypes() async {
    try {
      final response = await http.get(Uri.parse('${Endpoint.baseUrl}/api/v1/models/C_ProjectType?\$select=id,Name&\$orderby=Name'), headers: {'Content-Type': 'application/json', 'Authorization': Token.token});
      if (response.statusCode == 200) {
        final data = json.decode(utf8.decode(response.bodyBytes));
        return data['records'] ?? [];
      }
  ...
  Future<bool> createProject({required String value, required String name, required String description, int? bpId, String? dateContract, String? dateFinish, int? projectTypeId}) async {
    try {
      final createUrl = Uri.parse(Endpoint.project);
      final Map<String, dynamic> payload = {'Value': value, 'Name': name, 'Description': description};

      if (bpId != null) {
        payload['C_BPartner_ID'] = {'id': bpId};
      }
      if (dateContract != null && dateContract.isNotEmpty) {
        payload['DateContract'] = dateContract.contains('T') ? dateContract : '${dateContract}T00:00:00Z';
      }
      if (dateFinish != null && dateFinish.isNotEmpty) {
        payload['DateFinish'] = dateFinish.contains('T') ? dateFinish : '${dateFinish}T00:00:00Z';
      }
